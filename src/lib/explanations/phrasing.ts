...
export function groupAmenityLabels(amenityIds: string[]): {
  ...
  const leisure = labels.filter(a => ['beach bar', 'taverna', 'food', 'sunbeds', 'umbrellas', 'music'].includes(lower(a)));
  const activity = labels.filter(a => ['snorkeling', 'water sports', 'fishing', 'hiking', 'cliff jumping', 'boat trips', 'photography', 'birdwatching'].includes(lower(a)));
  return { essential, leisure, activity, allLabels: labels };
}

export type FacetKey = 'location' | 'type' | 'waves' | 'parking' | 'amenities' | 'blue_flag' | 'organization' | 'other';

export function getLocationPart(filters: FilterState, userLocation: GeolocationPosition | null): string | null {
  // Prefer explicit extracted locations array > single location > search term
  const locations: string[] | undefined = filters.locations && filters.locations.length > 0
    ? filters.locations
...
export function getWavesPart(filters: FilterState): string | null {
  ...
  return conditions.length === 1 ? `featuring ${conditions[0]}` : `with ${formatList(conditions, 'or')}`;
}
...
export function getOrganizationPart(filters: FilterState): string | null {
  ...
  const types = filters.organized.map(org => (org === 'organized' ? 'well-organized' : 'natural and unspoiled'));
