...
export class EntityRecognizer {
  ...
  private initializePatterns(): void {
    ...
    this.amenityPatterns.set(
      "sunbeds",
      /\b(sunbeds?|sun\s+beds?|beach\s+beds?|loungers?|beach\s+chairs?|deck\s+chairs?)\b/i
    );
    this.amenityPatterns.set(
      "umbrellas",
      /\b(umbrellas?|sun\s+umbrellas?|beach\s+umbrellas?|parasols?|shade\s+umbrellas?)\b/i
    );
    this.amenityPatterns.set(
      "showers",
      /\b(showers?|fresh\s+water\s+showers?|outdoor\s+showers?|beach\s+showers?|rinse\s+off)\b/i
    );
    this.amenityPatterns.set(
      "toilets",
      /\b(toilets?|restrooms?|bathrooms?|wc|washrooms?|facilities)\b/i
    );
    this.amenityPatterns.set(
      "lifeguard",
      /\b(lifeguards?|lifeguard\s+service|lifeguard\s+on\s+duty|lifesaving|safety\s+patrol|beach\s+safety)\b/i
    );
    this.amenityPatterns.set(
      "beach_bar",
      /\b(beach\s+bars?|beachside\s+bars?|seaside\s+bars?|beach\s+club|bar\s+on\s+beach)\b/i
    );
    this.amenityPatterns.set(
      "taverna",
      /\b(tavernas?|greek\s+tavernas?|traditional\s+tavernas?|restaurants?|greek\s+restaurants?|local\s+restaurants?)\b/i
    );
    this.amenityPatterns.set(
      "food",
      /\b(food|snacks?|refreshments?|cafes?|cafés?|eateries?|dining|meals?)\b/i
    );
    this.amenityPatterns.set(
      "music",
      /\b(music|live\s+music|dj|entertainment|beach\s+music|background\s+music)\b/i
    );
    this.amenityPatterns.set(
      "snorkeling",
      /\b(snorkeling|snorkelling|snorkel\s+gear|snorkel\s+equipment|underwater\s+exploration|marine\s+life)\b/i
    );
    this.amenityPatterns.set(
      "water_sports",
      /\b(water\s+sports|aquatic\s+activities|sea\s+sports|marine\s+activities|water\s+activities)\b/i
    );
    this.amenityPatterns.set(
      "family_friendly",
      /\b(family\s+friendly|family-friendly|good\s+for\s+families|child\s+friendly|kids\s+friendly|suitable\s+for\s+children|for\s+families|families)\b/i
    );
    this.amenityPatterns.set(
      "boat_trips",
      /\b(boat\s+trips?|boat\s+tours?|boat\s+excursions?|sailing|boat\s+rides?|marine\s+tours?)\b/i
    );
    this.amenityPatterns.set(
      "fishing",
      /\b(fishing|fish|fishing\s+spots?|good\s+for\s+fishing|anglers?|fishing\s+opportunities)\b/i
    );
    this.amenityPatterns.set(
      "photography",
      /\b(photography|photos?|pictures?|instagram|selfies?|instagrammable|scenic|picturesque|photo\s+opportunities|photogenic)\b/i
    );
    this.amenityPatterns.set(
      "hiking",
      /\b(hiking|hike|hiking\s+trails?|walking\s+trails?|nature\s+walks?|trekking|trails?)\b/i
    );
    this.amenityPatterns.set(
      "birdwatching",
      /\b(birdwatching|bird\s+watch|birds?|bird\s+spotting|ornithology|bird\s+observation)\b/i
    );
    this.amenityPatterns.set(
      "cliff_jumping",
      /\b(cliff\s+jumping|cliff\s+dive|cliff\s+diving|rock\s+jumping|high\s+diving|cliff\s+leaping)\b/i
    );

    // Beach type patterns - enhanced to match exact database values
    this.beachTypePatterns.set(
      "SANDY",
      /\b(white\s+sand|golden\s+sand|sandy\s+beach|sand\s+beach|sandy|sand|fine\s+sand|soft\s+sand|powder\s+sand|beach\s+sand)\b/i
    );
    this.beachTypePatterns.set(
      "PEBBLY",
      /\b(pebble\s+beach|stone\s+beach|stony\s+beach|pebbly|pebble|pebbles|stony|rocky\s+pebbles|small\s+stones|gravel)\b/i
    );
    this.beachTypePatterns.set(
      "MIXED",
      /\b(mixed\s+sand|sand\s+and\s+pebbles|mixed|combination|both\s+sand\s+and\s+pebbles|varied\s+surface)\b/i
    );
    this.beachTypePatterns.set(
      "OTHER",
      /\b(rocky\s+beach|concrete\s+beach|man\s+made|artificial\s+beach|rocky|rocks|rock|platform|deck|pier|marina)\b/i
    );

    // Wave condition patterns - enhanced to match exact database values
    this.waveConditionPatterns.set(
      "CALM",
      /\b(calm\s+water|calm\s+waters|calm\s+sea|still\s+water|peaceful\s+water|quiet\s+water|gentle\s+water|flat\s+water|shallow\s+water|shallow\s+waters|protected\s+beach|sheltered\s+beach|calm|still|peaceful|quiet|gentle|flat|shallow|mirror\s+like|glass\s+like|serene)\b/i
    );
    this.waveConditionPatterns.set(
      "MODERATE",
      /\b(moderate\s+waves|medium\s+waves|moderate|medium|normal\s+waves|average\s+waves|light\s+waves|small\s+waves)\b/i
    );
    this.waveConditionPatterns.set(
      "WAVY",
      /\b(big\s+waves|large\s+waves|strong\s+waves|rough\s+sea|choppy\s+water|wavy|waves|wave|choppy|rough|windy|powerful\s+waves|high\s+waves|stormy)\b/i
    );
    this.waveConditionPatterns.set(
      "SURFABLE",
      /\b(good\s+for\s+surfing|waves\s+for\s+surfing|surfing\s+beach|body\s+surf|bodysurf|surf|surfing|surfable|surf\s+spot|surf\s+break|wave\s+riding)\b/i
    );
    ...
    this.parkingPatterns.set(
      "NONE",
      /\b(no\s+parking|without\s+parking|no\s+car\s+access|no\s+vehicle\s+access|walk\s+only|no\s+parking\s+available)\b/i
    );
    this.parkingPatterns.set(
      "ROADSIDE",
      /\b(roadside\s+parking|street\s+parking|on\s+street\s+parking|road\s+parking|side\s+of\s+road|along\s+road)\b/i
    );
    this.parkingPatterns.set(
      "SMALL_LOT",
      /\b(limited\s+parking|small\s+parking|few\s+spaces|small\s+lot|limited\s+spaces|restricted\s+parking)\b/i
    );
    this.parkingPatterns.set(
      "LARGE_LOT",
      /\b(large\s+parking|ample\s+parking|plenty\s+of\s+parking|easy\s+parking|free\s+parking|large\s+lot|big\s+parking|extensive\s+parking|parking\s+lot|parking)\b/i
    );

    // Organization patterns
    this.organizationPatterns.set("organized", /\b(organized\s+beach|organized|organised)\b/i);
    this.organizationPatterns.set(
      "unorganized",
      /\b(unorganized\s+beach|unorganized|unorganised|not\s+organized|wild\s+beach|natural\s+beach|undeveloped\s+beach|wild|natural)\b/i
    );
  }
