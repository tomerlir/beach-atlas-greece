  ...
  private calculateFuzzyScore(query: string, candidate: string): number {
    try {
      ...
      const levenshteinScore =
        1 -
        distance.string.levenshtein(query, candidate) / Math.max(query.length, candidate.length);
      const jaccardScore = distance.set.jaccard(
        new Set(query.split("")),
      ...
      const weightedScore =
        jaroWinklerScore * 0.4 + jaroScore * 0.3 + levenshteinScore * 0.2 + jaccardScore * 0.1;
