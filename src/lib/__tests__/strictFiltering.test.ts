...
describe('Strict verified filtering semantics', () => {
  it('includes a beach that matches all type criteria; excludes those that do not', () => {
    ...
    const included = VERIFIED_BEACHES.filter(b => matchesFilters(b as any, filters));
    expect(included.every(b => b.type === 'SANDY')).toBe(true);

    const filtersPebbly: FilterState = { ...baseFilters, type: ['PEBBLY'] };
    const includedPebbly = VERIFIED_BEACHES.filter(b => matchesFilters(b as any, filtersPebbly));
  ...
  it('enforces wave conditions strictly', () => {
    ...
    const excluded = VERIFIED_BEACHES.filter(b => !matchesFilters(b as any, filters));
  ...
  it('requires all amenities to be present (AND semantics)', () => {
    ...
    expect(included.every(b => (b.amenities || []).includes('sunbeds') && (b.amenities || []).includes('beach_bar'))).toBe(true);

    const filtersStrict: FilterState = { ...baseFilters, amenities: ['lifeguard', 'beach_bar'] };
    const includedStrict = VERIFIED_BEACHES.filter(b => matchesFilters(b as any, filtersStrict));
    // Expect none if no beach has both
    expect(includedStrict.every(b => (b.amenities || []).includes('lifeguard') && (b.amenities || []).includes('beach_bar'))).toBe(true);
  });
