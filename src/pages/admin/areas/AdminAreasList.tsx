    ...
    try {
      ...
      if (areaIds.length > 0) {
        ...
        if (!beachError && beachData) {
          beachCounts = beachData.reduce((acc, beach) => {
            if (beach.area_id) {
      ...
      setTotal(count || 0);
      setLiveMsg(`Loaded ${areasWithCounts.length || 0} areas. Page ${page} of ${Math.max(1, Math.ceil((count||0)/PAGE_SIZE))}.`);
    } catch (err: any) {
        ...
        <table className="w-full text-sm">
          ...
          <thead>
            <tr className="text-left border-b">
              ...
              <th scope="col" className="py-2 pr-2">Slug</th>
              <th scope="col" className="py-2 pr-2">Beaches Count</th>
          ...
          <tbody>
            {items.map((area) => (
              <tr key={area.id} className="border-b last:border-0 align-top">
                <td className="py-2 pr-2 font-medium text-foreground">
                  <Link to={`/admin/areas/${area.id}`} className="underline underline-offset-2">{area.name}</Link>
                </td>
                <td className="py-2 pr-2 text-xs font-mono bg-muted/30 rounded px-1">{area.slug}</td>
                ...
                <td className="py-2 pr-2 whitespace-nowrap">{formatRelativeUpdatedAt(area.updated_at)}</td>
                <td className="py-2 pr-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Button 
                      variant="destructive" 
                      size="sm" 
                      onClick={(e)=>{ lastFocusedRef.current = e.currentTarget; onDelete(area.id, area.name, area.beach_count);} }
            ...
            {!loading && items.length === 0 && (
              <tr>
                <td colSpan={7} className="py-6 text-center text-muted-foreground">No results</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        {items.map((area) => (
          <Card key={area.id} className="p-3">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-semibold"><Link to={`/admin/areas/${area.id}`} className="underline underline-offset-2">{area.name}</Link></h3>
            ...
            <dl className="mt-2 grid grid-cols-2 gap-2 text-sm">
              <div>
                ...
                <dd><Badge variant="secondary">{area.beach_count}</Badge></dd>
              </div>
              <div>
            ...
            <div className="mt-3 flex gap-2">
              <Link to={`/admin/areas/${area.id}`} className="w-full"><Button variant="outline" className="w-full">Edit</Button></Link>
              <Button 
                variant="destructive" 
                className="w-full" 
                onClick={(e)=>{ lastFocusedRef.current = e.currentTarget; onDelete(area.id, area.name, area.beach_count);} }
