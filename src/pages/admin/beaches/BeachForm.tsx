...
const BeachForm: React.FC = () => {
  ...
  const ensureUniqueSlug = async (base: string, excludeId?: string): Promise<string> => {
    ...
    const taken = new Set<string>(
      (data || []).filter((r) => !excludeId || r.id !== excludeId).map((r) => r.slug)
    );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!parsed.success) {
      ...
      toast({
        ...
        description: `First invalid: ${firstField}`,
        variant: "destructive",
      });
    ...
    try {
      ...
      if (/slug.*unique/i.test(msg)) {
        ...
        try {
          ...
          toast({
            title: "Saved",
            description: "Saved without slug (schema not ready).",
            variant: "default",
          });
          setDirty(false);
          clearDraft(); // Clear draft state after successful save
    ...
    <form onSubmit={handleSubmit} className="space-y-4" onChange={() => setDirty(true)}>
      <div className="flex items-center gap-2">
        <h1 className="text-2xl font-semibold text-foreground">
          {mode === "create" ? "Add Beach" : "Edit Beach"}
        </h1>
        {hasStoredDraftData() && (
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          ...
          <Input
            ...
            ref={(el) => {
              if (errors.name) firstInvalidRef.current = el;
              nameInputRef.current = el;
            }}
          />
          {errors.name && (
            <p id="name-err" className="text-sm text-destructive">
              {errors.name}
            </p>
          )}
        </div>
        <div>
          ...
          >
            <SelectTrigger
              ...
              aria-describedby={errors.area_id ? "area-err" : undefined}
            >
          ...
          {errors.area_id && (
            <p id="area-err" className="text-sm text-destructive">
              {errors.area_id}
            </p>
          )}
        </div>
        <div>
          ...
          />
          {errors.latitude && (
            <p id="lat-err" className="text-sm text-destructive">
              {errors.latitude}
            </p>
          )}
        </div>
        <div>
          ...
          />
          {errors.longitude && (
            <p id="lng-err" className="text-sm text-destructive">
              {errors.longitude}
            </p>
          )}
        </div>
        <div className="md:col-span-2">
          ...
          />
          {errors.description && (
            <p id="desc-err" className="text-sm text-destructive">
              {errors.description}
            </p>
          )}
        </div>
        ...
        <div className="md:col-span-2">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Add proper attribution for the photo. Include license information and source when
              available.
            </p>
          </div>
        </div>
        ...
        <div className="md:col-span-2">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Optional: Where this beach data came from.
          </p>
        </div>
        <div className="md:col-span-2">
          <details>
            ...
            <div className="mt-2">
              ...
              />
              {errors.slug && (
                <p id="slug-err" className="text-sm text-destructive">
                  {errors.slug}
                </p>
              )}
              <p className="text-xs text-muted-foreground mt-1">
