...
export default function ResultsSummary({
    ...
    useEffect(() => {
        const isRelaxed = !!filters.search && resultCount > 0 && resultCount < 10; // Simple heuristic
                ...
                <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="text-muted-foreground min-w-0 flex-1">
                        <div className="text-sm flex flex-col gap-1">
                            <span className="font-medium text-foreground" aria-live="polite" aria-label={hasExplanation ? `${getResultCountMessage()}, ${explanation.fullText}` : getResultCountMessage()}>
                                {getResultCountMessage()}
                                {hasExplanation && (
                                    <span className="font-normal text-muted-foreground ml-1">
                                        {explanationParts.join(', ')}
                                        {explanation.secondaryCount > 0 ? `, and ${explanation.secondaryCount} more criteria` : ''}
                                    </span>
                                )}
                            </span>
