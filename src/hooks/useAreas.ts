...
export const useNearbyAreas = (
  ...
  const nearby = useMemo(() => {
    ...
    if (!current || current.centroid_latitude == null || current.centroid_longitude == null) return [];

    return centroids
      .filter(
        (a) =>
          a.id !== currentAreaId &&
          a.centroid_latitude != null &&
          a.centroid_longitude != null
      )
      .map((a) => ({
        area: a,
