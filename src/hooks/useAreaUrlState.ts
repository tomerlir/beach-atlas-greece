...
export function useAreaUrlState(areaName: string) {
  const [searchParams, setSearchParams] = useSearchParams();

  const filters = useMemo(() => {
    const state: AreaFilterState = { ...defaultFilters, area: areaName };
  ...
  const updateFilters = useCallback((updates: Partial<Omit<AreaFilterState, 'area'>>) => {
